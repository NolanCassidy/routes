...
def dfs(place, dist_so_far, roads, distances):
    ...
    """
    if distances == {}:             # sets first node to zero
        distances[place] = 0.0
    for road in roads[place]:
        n,d = road
        dist_so_far = distances[place]
        dist_so_far += d
        if n not in distances or dist_so_far < distances[n]:
            distances[n] = dist_so_far
            dfs(n, dist_so_far, roads, distances)
